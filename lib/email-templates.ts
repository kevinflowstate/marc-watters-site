...
export async function sendWelcomeEmail(to: string, name: string, activationUrl: string) {
  ...
  const resend = await getResend(); return resend.emails.send({
    ...
    html: wrap(`
      ...
      <p style="color: #555; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">
        Your client portal is set up and ready to go. Click below to create your password and activate your account. This link stays active until you use it.
      </p>
      ${button(activationUrl, "Activate Your Account")}
    `),
  });
}
