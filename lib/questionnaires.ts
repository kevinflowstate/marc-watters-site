...
export const BUSINESS_HEALTH_CHECKLIST_TYPE: QuestionnaireType = "business_health_checklist";

export const businessHealthChecklistConfig: QuestionnaireFormConfig = {
...
export function isSupportedQuestionnaireType(value: string): value is QuestionnaireType {
  return value === BUSINESS_HEALTH_CHECKLIST_TYPE;
}

export function getQuestionnaireConfig(type: QuestionnaireType): QuestionnaireFormConfig {
  if (type === BUSINESS_HEALTH_CHECKLIST_TYPE) return businessHealthChecklistConfig;
  return businessHealthChecklistConfig;
}
...
export function getQuestionAnswerLabel(
  ...
  if (question.type === "single_choice" && question.options?.length) {
    ...
    if (question.allow_other && rawValue === "other") {
      const otherValue = responses[`${question.id}__other`];
      return otherValue ? `${option.label}: ${otherValue}` : option.label;
    }
